...
func (q *Queue)nodes() []*Node{
	var ret []*Node
	for _, d := range q.data{
		ret = append(ret, &Node{ID: q.mapper.Map(d.status.GetHostname()), Hostname: d.status.GetHostname(), IP: d.status.GetNodeStatus().GetAddresses()[0].Format()})
	}
	return ret
}
...
func (q *Queue)edgesOnOneNode(node *spec.StatusManagerPeerStatus) []*Edge{
	...
	for _, d := range node.GetPeerStatus(){
		if d.GetBgpStatus().GetBGPState() == spec.BGPStates_BGPStateEstablished{
			ret = append(ret, &Edge{ID: q.mapper.Map(node.GetHostname()+"-"+d.GetRemoteHostname()), SourceID: q.mapper.Map(node.GetHostname()), TargetID: q.mapper.Map(d.GetRemoteHostname())})
		}
	}
	return ret
}
